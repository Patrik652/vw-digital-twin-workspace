...
PROCESSED_COUNTER = Counter("messages_processed_total", "Total telemetry messages processed")
...
async def detect(batch: TelemetryBatch) -> DetectionResult:
    ...
    for item in batch.telemetry:
        ...
        anomalies.extend(
            _detector.detect_zscore("spindle.temperature_c", telemetry["spindle"]["temperature_c"], item.machine_id)
        )
    ...
    return DetectionResult(anomalies=anomalies, model_not_ready=not _detector._iforest_ready, processed=len(batch.telemetry))
