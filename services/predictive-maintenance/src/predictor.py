...
from models import MaintenancePrediction, MaintenanceScheduleResponse, SpindleHealthResponse, ToolRULResponse
...
def predict_tool_rul(
    ...
    consumed = (runtime_minutes * 0.6) + (wear_factor * base_life) + (speed_factor * 20.0)
    remaining = max(0.0, base_life - consumed)
    confidence = max(0.3, 1.0 - wear_factor)
    return ToolRULResponse(machine_id=machine_id, minutes_remaining=remaining, confidence=confidence)
...
def predict_spindle_health(
    ...
    health_score = max(0.0, 100.0 * (1.0 - (0.5 * vib_ratio + 0.4 * temp_ratio + 0.1 * trend_penalty)))
...
def build_maintenance_schedule(
    ...
    tool_urgency = "high" if tool_rul.minutes_remaining < 30 else "medium" if tool_rul.minutes_remaining < 120 else "low"
    predictions.append(
        MaintenancePrediction(
            component="Tool",
            action="Replace",
            urgency=tool_urgency,
            estimated_time=(now + timedelta(minutes=tool_rul.minutes_remaining)).isoformat(),
            confidence=tool_rul.confidence,
        )
    )

    spindle_urgency = "high" if spindle_health.health_score < 40 else "medium" if spindle_health.health_score < 70 else "low"
    predictions.append(
        MaintenancePrediction(
            component="Spindle Bearings",
            action="Inspect",
            urgency=spindle_urgency,
            estimated_time=(now + timedelta(days=spindle_health.days_to_maintenance)).isoformat(),
            confidence=0.7,
        )
    )
